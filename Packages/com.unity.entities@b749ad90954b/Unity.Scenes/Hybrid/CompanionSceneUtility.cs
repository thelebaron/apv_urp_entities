...
#endif
        [MenuItem("Tools/LiveConversion/Log SceneCompanions")]
        static void LogRootSceneCompanions()
        {
            var rootObjects = _companionScene.GetRootGameObjects();
            UnityEngine.Debug.Log($" _companionScene rootObjects: {rootObjects.Length}");
            foreach (var rootObject in rootObjects)
            {
                UnityEngine.Debug.Log($" root: {rootObject.name}");
            }
            
            rootObjects = _companionSceneLiveConversion.GetRootGameObjects();
            UnityEngine.Debug.Log($" _companionSceneLiveConversion rootObjects: {rootObjects.Length}");
            foreach (var rootObject in rootObjects)
            {
                UnityEngine.Debug.Log($" root: {rootObject.name}");
            }
