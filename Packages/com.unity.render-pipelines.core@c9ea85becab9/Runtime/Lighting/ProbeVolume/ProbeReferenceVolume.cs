...
#pragma warning restore 618
        ...
        public void SetActiveBakingSet(ProbeVolumeBakingSet bakingSet)
        {
            ...
            {
                if(bakingSet!=null)
                    Debug.Log($"Baking set {bakingSet.name} is already active");
                return;
            }
            ...
            {
                if(bakingSet!=null)
                    Debug.Log($"Baking set {bakingSet.name} is scheduled to be loaded after initialization.");
                return;
            }
...
#pragma warning restore 618
        ...
        internal void AddPendingSceneLoading(string sceneGUID, ProbeVolumeBakingSet bakingSet)
        {
            ...
            {
                Debug.Log($"User might have loaded other scenes with probe volumes but not belonging to the \"single scene\" baking set..");
                return;
            }
            ...
            if (m_CurrentBakingSet != null && bakingSet != m_CurrentBakingSet)
            {
                ...
                Debug.Log($"Trying to load data for a scene from a different baking set than currently loaded ones..");
                return;
            }
            ...
            //Debug.Log($"m_PendingScenesToBeLoaded.Add(sceneGUID, (bakingSet, bakingSet.GetSceneCellIndexList(sceneGUID)));");
        ...
        {
            //Debug.Log( "PerformPendingLoading()");

            if ((m_PendingScenesToBeLoaded.Count == 0 && m_ActiveScenes.Count == 0) || !m_NeedLoadAsset || !m_ProbeReferenceVolumeInit)
            {
                //Debug.Log($"PerformPendingLoading() early return {m_PendingScenesToBeLoaded.Count == 0 && m_ActiveScenes.Count == 0} {m_NeedLoadAsset} {m_ProbeReferenceVolumeInit}");
                return;
            }
            ...
            //Debug.Log($"PerformPendingLoading()  Mark the loading as done.");
        }
