...
namespace UnityEngine.Rendering
{
    ...
    public partial class AdaptiveProbeVolumes
    {
        ...
        static bool SetBakingContext(List<ProbeVolumePerSceneData> perSceneData)
        {
            ...
            for (int i = 0; i < perSceneData.Count; ++i)
            {
                //Debug.Log( "perSceneData.Count " + perSceneData.Count);
                var data = perSceneData[i];
                data.gameObject.hideFlags = HideFlags.None;
                var scene = data.gameObject.scene;
                ...
                {
                    data.gameObject.name = "Problematic go";
                    Debug.Log($"data.gameObject scenename { data.gameObject.scene.name} isloaded {data.gameObject.scene.isLoaded} isSubscene {data.gameObject.scene.isSubScene}  {data.gameObject.scene.path}");
